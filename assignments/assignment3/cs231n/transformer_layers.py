...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        i_term = torch.arange(max_len)
        j_term = torch.exp(-torch.arange(0, embed_dim, 2) / embed_dim * torch.log(torch.tensor(1e4)))
        pe[0, :, ::2] = torch.sin(torch.outer(i_term, j_term))
        pe[0, :, 1::2] = torch.cos(torch.outer(i_term, j_term))
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        k = self.key(key)
        v = self.value(value)
        q = q.view(N, S, H, E // H).permute(0, 2, 1, 3)
        k = k.view(N, T, H, E // H).permute(0, 2, 3, 1)
        v = v.view(N, T, H, E // H).permute(0, 2, 1, 3)
        attn_scores = q @ k / (E // H)**0.5
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        n_per_dim = self.img_size // self.patch_size
        x = x.reshape(N, C, n_per_dim, patch_size, n_per_dim, patch_size).permute(0, 2, 4, 1, 3, 5)
